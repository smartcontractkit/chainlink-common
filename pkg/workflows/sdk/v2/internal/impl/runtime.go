...
type CallCapability func(req unsafe.Pointer, reqLen int32, id unsafe.Pointer) int64
type AwaitCapabilities func(awaitRequest unsafe.Pointer, awaitRequestLen int32, responseBuffer unsafe.Pointer, maxResponseLen int32) int64

func NewDonRuntime(callCapability CallCapability, capabilities AwaitCapabilities, writer io.Writer) sdk.DonRuntime {
	return &donRuntime{
		runtimeBase: runtimeBase{
			callCapability:    callCapability,
			awaitCapabilities: capabilities,
			writer:            writer,
		},
	}
}

func NewNodeRuntime(callCapability CallCapability, capabilities AwaitCapabilities, writer io.Writer) sdk.NodeRuntime {
	return &nodeRuntime{
		runtimeBase: runtimeBase{
			callCapability:    callCapability,
			awaitCapabilities: capabilities,
...
func (r *runtimeBase) CallCapability(request *sdkpb.CapabilityRequest) sdk.Promise[*sdkpb.CapabilityResponse] {
	...
	result := r.callCapability(marshalledPtr, marshalledLen, unsafe.Pointer(&id[0]))
	if result < 0 {
