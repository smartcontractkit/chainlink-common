...
func TestEmitDelta_Success(t *testing.T) {
	...
	rm.EmitDelta(t.Context(), testIdentity, -3, UtilizationFields{
		ResourceType: "operations",
		ResourceID:   "trigger-1",
		OrgID:        "org-1",
	})
	...
	// event_id is generated by the manager, not the caller.
	assert.NotEmpty(t, record.GetUtilizations()[0].GetEventId())
...
func TestEmitUsage_Action(t *testing.T) {
	...
	rm.EmitUsage(t.Context(), testIdentity, 7, UtilizationFields{
		ResourceType: "operations",
		ResourceID:   "req-1",
		OrgID:        "org-1",
	})
...
func TestEmitDelta_EventIDUniquePerEmission(t *testing.T) {
	emitter := &fakeEmitter{}
	rm := NewResourceManager(logger.Test(t), ResourceManagerConfig{
		MeterRecordsEnabled: true,
		Emitter:             emitter,
	...
	rm.EmitDelta(t.Context(), testIdentity, 1, fields)
	rm.EmitDelta(t.Context(), testIdentity, 1, fields)

	require.Len(t, emitter.calls, 2)
	var r0, r1 meteringpb.MeterRecord
	require.NoError(t, proto.Unmarshal(emitter.calls[0].body, &r0))
	require.NoError(t, proto.Unmarshal(emitter.calls[1].body, &r1))
	id0 := r0.GetUtilizations()[0].GetEventId()
	id1 := r1.GetUtilizations()[0].GetEventId()
...
func TestEmitSnapshot_OnePerResource(t *testing.T) {
	...
	for _, s := range snaps {
		...
		// event_id is stamped by the manager on snapshot utilizations too.
		assert.NotEmpty(t, s.GetUtilization()[0].GetEventId())
	}
