...
// Driver is a compiler enforced type used that maps to database driver names
type Driver string
...
const (
	...
	DriverTxWrappedPostgres Driver = "txdb"
)
...
func (config ConnectionConfig) NewDB(ctx context.Context, uri string, driver Driver) (*sqlx.DB, error) {
	if driver == DriverTxWrappedPostgres {
		...
		return config.open(ctx, uuid.NewString(), string(driver))
	}
	return config.openDB(ctx, uri, string(driver))
}

func (config ConnectionConfig) open(ctx context.Context, uri string, driverName string) (*sqlx.DB, error) {
...
func (config ConnectionConfig) openDB(ctx context.Context, uri string, driverName string) (*sqlx.DB, error) {
...
func (config ConnectionConfig) initSQL() string {
	return fmt.Sprintf(`SET TIME ZONE 'UTC'; SET lock_timeout = %d; SET idle_in_transaction_session_timeout = %d; SET default_transaction_isolation = %q`,
		config.DefaultLockTimeout.Milliseconds(), config.DefaultIdleInTxSessionTimeout.Milliseconds(), defaultIsolation)
}

func (config ConnectionConfig) newDB(ctx context.Context, sqldb *sql.DB, driverName string) (*sqlx.DB, error) {
