...
type BasicActionCapability interface {
	PerformAction(ctx context.Context, metadata capabilities.RequestMetadata, input *nodeaction.NodeInputs) (*capabilities.ResponseAndMetadata[*nodeaction.NodeOutputs], caperrors.Error)
...
func (c *BasicActionServer) Initialise(ctx context.Context, dependencies core.StandardCapabilitiesDependencies) error {
	initErr := c.BasicActionCapability.Initialise(ctx, dependencies)
	initAttrs := []attribute.KeyValue{attribute.String("capability", "basic-test-node-action@1.0.0")}
	...
	if err := dependencies.CapabilityRegistry.Add(ctx, &basicActionCapability{
		...
		return fmt.Errorf("error when adding %s to the registry: %w", "basic-test-node-action@1.0.0", err)
	}
...
func (c *basicActionCapability) Execute(ctx context.Context, request capabilities.CapabilityRequest) (capabilities.CapabilityResponse, error) {
	...
	case "PerformAction":
		...
		wrapped := func(ctx context.Context, metadata capabilities.RequestMetadata, input *nodeaction.NodeInputs, _ *emptypb.Empty) (*nodeaction.NodeOutputs, capabilities.ResponseMetadata, *capabilities.OCRAttestation, error) {
			output, err := c.BasicActionCapability.PerformAction(ctx, metadata, input)
			if err != nil {
				return nil, capabilities.ResponseMetadata{}, nil, err
			}
			if output == nil {
				return nil, capabilities.ResponseMetadata{}, nil, fmt.Errorf("output and error is nil for method PerformAction(..) (if output is nil error must be present)")
			}
