...
// Use New to construct a NodeJWTAuthenticator from this config.
...
type NodeJWTAuthenticator struct {
	...
	logger           nodeJWTLogger
}

// Deprecated: use NodeJWTAuthenticatorConfig.New.
func NewNodeJWTAuthenticator(nodeAuthProvider NodeAuthProvider, slogger *slog.Logger, config ...*NodeJWTAuthenticatorConfig) *NodeJWTAuthenticator {
	resolved := resolveNodeJWTConfig(config...)
	if resolved.SLogger == nil {
		resolved.SLogger = slogger
	}
	return resolved.New(nodeAuthProvider)
}

// New constructs a NodeJWTAuthenticator from the config.
func (config NodeJWTAuthenticatorConfig) New(nodeAuthProvider NodeAuthProvider) *NodeJWTAuthenticator {
...
func resolveNodeJWTConfig(configs ...*NodeJWTAuthenticatorConfig) NodeJWTAuthenticatorConfig {
	var resolved NodeJWTAuthenticatorConfig
	for _, cfg := range configs {
		if cfg == nil {
			continue
		}
		if cfg.SLogger != nil {
			resolved.SLogger = cfg.SLogger
		}
		if cfg.Logger != nil {
			resolved.Logger = cfg.Logger
		}
		if cfg.Leeway > 0 {
			resolved.Leeway = cfg.Leeway
		}
	}
	return resolved
}

func (config NodeJWTAuthenticatorConfig) buildLogger() nodeJWTLogger {
	if config.Logger != nil {
		return newChainlinkLoggerAdapter(config.Logger)
	}
	if config.SLogger != nil {
		return newSlogLoggerAdapter(config.SLogger)
	}
