...
func createTestBinary(outputPath, path string, uncompressed bool, t *testing.T) []byte {
	cmd := exec.Command("/Users/ryantinianov/go/pkg/mod/golang.org/toolchain@v0.0.1-go1.23.3.darwin-arm64/bin/go", "build", "-o", path, fmt.Sprintf("github.com/smartcontractkit/chainlink-common/pkg/workflows/wasm/legacy/host/%s", outputPath)) // #nosec
