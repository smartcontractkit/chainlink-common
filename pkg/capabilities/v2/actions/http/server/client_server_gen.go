...
type ClientCapability interface {
	SendRequest(ctx context.Context, metadata capabilities.RequestMetadata, input *http.Request) (*capabilities.ResponseAndMetadata[*http.Response], caperrors.Error)
...
func (c *ClientServer) Initialise(ctx context.Context, dependencies core.StandardCapabilitiesDependencies) error {
	...
	initAttrs := []attribute.KeyValue{attribute.String("capability", "http-actions@1.0.0-alpha")}
	...
	if err := dependencies.CapabilityRegistry.Add(ctx, &clientCapability{
		...
		return fmt.Errorf("error when adding %s to the registry: %w", "http-actions@1.0.0-alpha", err)
	}
...
func (c *clientCapability) Execute(ctx context.Context, request capabilities.CapabilityRequest) (capabilities.CapabilityResponse, error) {
	...
	case "SendRequest":
		...
		wrapped := func(ctx context.Context, metadata capabilities.RequestMetadata, input *http.Request, _ *emptypb.Empty) (*http.Response, capabilities.ResponseMetadata, *capabilities.OCRAttestation, error) {
