...
type ModifiedConfig struct {
	...
	Encoder                 ocr3cap.Encoder
	EncoderConfig           ocr3cap.EncoderConfig `yaml:"encoder_config" json:"encoder_config"`
	ChainWriter             *chainwriter.TargetConfig
	TargetChain             string
}
...
func NewWorkflowRemapped(rawConfig []byte) (*sdk.WorkflowSpecFactory, error) {
	...
	consensus := ocr3Config.New(workflow, "ccip_feeds", ocr3cap.DataFeedsConsensusInput{
		Observations: sdk.ListOf[streams.Feed](streamsTrigger),
	})
...
func NewWorkflowSpecFromPrimitives(rawConfig []byte) (*sdk.WorkflowSpecFactory, error) {
	...
	ocrConfig := ocr3cap.DataFeedsConsensusConfig{
		AggregationConfig: ocr3cap.DataFeedsConsensusConfigAggregationConfig{
	...
	consensus := ocrConfig.New(workflow, "data-feeds-report", ocr3cap.DataFeedsConsensusInput{
...
func TestBuilder_ValidSpec(t *testing.T) {
	...
	t.Run("duplicate names causes errors", func(t *testing.T) {
		...
		consensus2 := conf.Ocr.New(workflow, "ccip_feeds", ocr3cap.DataFeedsConsensusInput{
			Observations: sdk.ListOf[streams.Feed](streamsTrigger)},
		)
	...
	t.Run("empty ref causes an error", func(t *testing.T) {
		...
		workflow := sdk.NewWorkflowSpecFactory(conf.Workflow)
		streamsTrigger := conf.Streams.New(workflow)
		consensus := conf.Ocr.New(workflow, "", ocr3cap.DataFeedsConsensusInput{
			Observations: sdk.ListOf[streams.Feed](streamsTrigger)},
		)
	...
	t.Run("Capabilities can be used multiple times with different references", func(t *testing.T) {
		...
		consensus2 := conf.Ocr.New(workflow, "ccip_feeds_different", ocr3cap.DataFeedsConsensusInput{
			Observations: sdk.ListOf[streams.Feed](streamsTrigger)},
		)
...
func runSepoliaStagingTest(t *testing.T, config []byte, gen func([]byte) (*sdk.WorkflowSpecFactory, error)) {
	...
	assertWorkflowSpec(t, expectedSpec.WorkflowSpec, testWorkflowSpec)
}
...
type ModifiedConsensusConfig struct {
	...
	AggregationMethod       ocr3cap.DataFeedsConsensusConfigAggregationMethod `json:"aggregation_method" yaml:"aggregation_method" mapstructure:"aggregation_method"`
	Encoder                 ocr3cap.Encoder                                   `json:"encoder" yaml:"encoder" mapstructure:"encoder"`
	EncoderConfig           ocr3cap.EncoderConfig                             `json:"encoder_config" yaml:"encoder_config" mapstructure:"encoder_config"`
