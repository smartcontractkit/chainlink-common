...
type BasicActionCapability interface {
	PerformAction(ctx context.Context, metadata capabilities.RequestMetadata, input *nodeaction.NodeInputs /* TODO  config nodeaction.NodeInputs*/) (*nodeaction.NodeOutputs, error)
...
func (cs *basicActionServer) Close() error {
	...
	defer cancel()
	if err := cs.capabilityRegistry.Remove(ctx, "node-action@1.0.0"); err != nil {
		return err
	}
...
func (c *basicActionCapability) Info(ctx context.Context) (capabilities.CapabilityInfo, error) {
	...
	return capabilities.NewCapabilityInfo("node-action@1.0.0", capabilities.CapabilityTypeAction, c.BasicActionCapability.Description())
}
