...
type ClientCapability interface {
	...
	RegisterOnFinalityViolation(ctx context.Context, triggerID string, metadata capabilities.RequestMetadata, input *emptypb.Empty) (<-chan capabilities.TriggerAndId[*crosschain.BlockRange], error)
	UnregisterOnFinalityViolation(ctx context.Context, triggerID string, metadata capabilities.RequestMetadata, input *emptypb.Empty) error
...
func (c *clientCapability) RegisterTrigger(ctx context.Context, request capabilities.TriggerRegistrationRequest) (<-chan capabilities.TriggerResponse, error) {
	...
	case "OnFinalityViolation":
		input := &emptypb.Empty{}
		return capabilities.RegisterTrigger(ctx, c.stopCh, "evm@1.0.0", request, input, c.ClientCapability.RegisterOnFinalityViolation)
	default:
...
func (c *clientCapability) UnregisterTrigger(ctx context.Context, request capabilities.TriggerRegistrationRequest) error {
	...
	case "OnFinalityViolation":
		...
		return c.ClientCapability.UnregisterOnFinalityViolation(ctx, request.TriggerID, request.Metadata, input)
	default:
