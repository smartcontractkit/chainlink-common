...
var file_relayer_proto_rawDesc = []byte{
	...
	0x42, 0x79, 0x74, 0x65, 0x73, 0x52, 0x06, 0x72, 0x65, 0x74, 0x56, 0x61, 0x6c, 0x22, 0x57, 0x0a,
	...
	0x63, 0x6f, 0x75, 0x6e, 0x74, 0x52, 0x65, 0x70, 0x6c, 0x79, 0x22, 0x00, 0x32, 0x59, 0x0a, 0x0b,
	0x43, 0x68, 0x61, 0x69, 0x6e, 0x52, 0x65, 0x61, 0x64, 0x65, 0x72, 0x12, 0x4a, 0x0a, 0x0e, 0x47,
...
var file_relayer_proto_goTypes = []interface{}{
	...
	(*BoundContract)(nil),                     // 43: loop.BoundContract
...
var file_relayer_proto_depIdxs = []int32{
	...
	46, // 7: loop.ObserveReply.value:type_name -> loop.BigInt
	...
	43, // 13: loop.GetLatestValueRequest.bc:type_name -> loop.BoundContract
	...
	41, // 37: loop.ChainReader.GetLatestValue:input_type -> loop.GetLatestValueRequest
	50, // 38: loop.Service.Name:input_type -> google.protobuf.Empty
	...
	10, // 45: loop.Relayer.NewConfigProvider:output_type -> loop.NewConfigProviderReply
	8,  // 46: loop.Relayer.NewPluginProvider:output_type -> loop.NewPluginProviderReply
	12, // 47: loop.Relayer.GetChainStatus:output_type -> loop.GetChainStatusReply
	15, // 48: loop.Relayer.ListNodeStatuses:output_type -> loop.ListNodeStatusesReply
	50, // 49: loop.Relayer.Transact:output_type -> google.protobuf.Empty
	19, // 50: loop.DataSource.Observe:output_type -> loop.ObserveReply
	22, // 51: loop.OffchainConfigDigester.ConfigDigest:output_type -> loop.ConfigDigestReply
	24, // 52: loop.OffchainConfigDigester.ConfigDigestPrefix:output_type -> loop.ConfigDigestPrefixReply
	26, // 53: loop.ContractConfigTracker.LatestConfigDetails:output_type -> loop.LatestConfigDetailsReply
	28, // 54: loop.ContractConfigTracker.LatestConfig:output_type -> loop.LatestConfigReply
	30, // 55: loop.ContractConfigTracker.LatestBlockHeight:output_type -> loop.LatestBlockHeightReply
	35, // 56: loop.ContractTransmitter.Transmit:output_type -> loop.TransmitReply
	37, // 57: loop.ContractTransmitter.LatestConfigDigestAndEpoch:output_type -> loop.LatestConfigDigestAndEpochReply
	39, // 58: loop.ContractTransmitter.FromAccount:output_type -> loop.FromAccountReply
	42, // 59: loop.ChainReader.GetLatestValue:output_type -> loop.GetLatestValueReply
	44, // 60: loop.Service.Name:output_type -> loop.NameReply
	50, // 61: loop.Service.Close:output_type -> google.protobuf.Empty
