...
func Test_Handler_SendsResponse(t *testing.T) {
	...
	h := requests.NewHandler(lggr, requests.NewStore[*testRequest](), clockwork.NewFakeClockAt(time.Now()), 1*time.Second)
	servicetest.Run(t, h)

	responseCh := make(chan testResponse, 10)
	h.SendRequest(ctx, &testRequest{
	...
	h.SendResponse(ctx, testResponse{
		WorkflowExecutionID: "test",
		Value:               testVal,
		Err:                 nil,
	})
...
func Test_Handler_PendingRequestsExpiry(t *testing.T) {
	...
	h := requests.NewHandler(lggr, requests.NewStore[*testRequest](), clock, 1*time.Second)
	servicetest.Run(t, h)
