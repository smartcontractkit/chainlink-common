...
// have written, so a run against any directory outside this module resolves them the compiled way
// - no package to parse and no file to write.
type dedupeShared struct {
	Value string
}

func (dedupeShared) DocComments() map[string]FieldDoc {
	return map[string]FieldDoc{"Value": {Comment: "Value is documented."}}
}
...
func (dedupeEmbedded) DocComments() map[string]FieldDoc {
	return map[string]FieldDoc{"Flat": {Comment: "Flat is documented."}}
}
...
func (dedupeAlpha) DocComments() map[string]FieldDoc {
	return map[string]FieldDoc{"Direct": {Comment: "Direct is documented."}}
}
...
func (dedupeBeta) DocComments() map[string]FieldDoc {
	return map[string]FieldDoc{"Shared": {Comment: "Shared is documented."}}
}
...
func TestFilesProducesEachTypeOnce(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			// A directory outside this module makes every type above a dependency's, so
			// nothing is parsed and nothing needs to be on disk.
			...
			// A duplicate would reach the generated source as two methods on one receiver,
			// which does not compile, so the guarantee is checked where it is consumed. The
			// package clause is supplied because only a local package carries one.
			local := captured[0]
			local.Name = "example"
...
// TestFilesOrderDoesNotChangeTheResult pins what a CI diff relies on: the same types produce the
// same output however the roots were listed.
func TestFilesOrderDoesNotChangeTheResult(t *testing.T) {
...
// which is how a duplicate shows up.
