...
func Authenticate(cfg config.WebhookConfig) gin.HandlerFunc {
	return func(c *gin.Context) {
		reqAccessKey := c.GetHeader(webhookAccessKeyHeader)
		reqSecret := c.GetHeader(webhookSecretHeader)
		// fetch key + secret each time (can change when POST /keys is used)
		if reqAccessKey == cfg.CIKey() && reqSecret == cfg.CISecret() {
			c.Next()
		} else {
