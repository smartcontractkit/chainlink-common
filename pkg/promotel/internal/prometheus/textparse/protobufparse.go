...
func NewProtobufParser(b []byte, parseClassicHistograms bool, st *labels.SymbolTable) promtextparse.Parser {
...
func (p *ProtobufParser) Exemplar(ex *exemplar.Exemplar) bool {
	if p.exemplarReturned && p.state == promtextparse.EntrySeries {
	...
	case dto.MetricType_HISTOGRAM, dto.MetricType_GAUGE_HISTOGRAM:
		isClassic := p.state == promtextparse.EntrySeries
...
// read.
func (p *ProtobufParser) Next() (promtextparse.Entry, error) {
	p.exemplarReturned = false
	switch p.state {
	case promtextparse.EntryInvalid:
		p.metricPos = 0
		p.fieldPos = -2
		...
		if !model.IsValidMetricName(model.LabelValue(name)) {
			return promtextparse.EntryInvalid, fmt.Errorf("invalid metric name: %s", name)
		}
		if help := p.mf.GetHelp(); !utf8.ValidString(help) {
			return promtextparse.EntryInvalid, fmt.Errorf("invalid help for metric %q: %s", name, help)
		}
		switch p.mf.GetType() {
		case dto.MetricType_COUNTER,
			...
			// All good.
		default:
			return promtextparse.EntryInvalid, fmt.Errorf("unknown metric type for metric %q: %s", name, p.mf.GetType())
		}
		unit := p.mf.GetUnit()
		if len(unit) > 0 {
			if p.mf.GetType() == dto.MetricType_COUNTER && strings.HasSuffix(name, "_total") {
				if !strings.HasSuffix(name[:len(name)-6], unit) || len(name)-6 < len(unit)+1 || name[len(name)-6-len(unit)-1] != '_' {
					return promtextparse.EntryInvalid, fmt.Errorf("unit %q not a suffix of counter %q", unit, name)
				}
			} else if !strings.HasSuffix(name, unit) || len(name) < len(unit)+1 || name[len(name)-len(unit)-1] != '_' {
				return promtextparse.EntryInvalid, fmt.Errorf("unit %q not a suffix of metric %q", unit, name)
			}
		}
		p.metricBytes.Reset()
		p.metricBytes.WriteString(name)

		p.state = promtextparse.EntryHelp
	case promtextparse.EntryHelp:
		p.state = promtextparse.EntryType
	case promtextparse.EntryType:
		t := p.mf.GetType()
		if (t == dto.MetricType_HISTOGRAM || t == dto.MetricType_GAUGE_HISTOGRAM) &&
			isNativeHistogram(p.mf.GetMetric()[0].GetHistogram()) {
			p.state = promtextparse.EntryHistogram
		} else {
			p.state = promtextparse.EntrySeries
		}
		if err := p.updateMetricBytes(); err != nil {
			return promtextparse.EntryInvalid, err
		}
	case promtextparse.EntryHistogram, promtextparse.EntrySeries:
		if p.redoClassic {
			p.redoClassic = false
			p.state = promtextparse.EntrySeries
			p.fieldPos = -3
		...
		t := p.mf.GetType()
		if p.state == promtextparse.EntrySeries && !p.fieldsDone &&
			...
			if p.state == promtextparse.EntrySeries &&
		...
		if p.metricPos >= len(p.mf.GetMetric()) {
			p.state = promtextparse.EntryInvalid
			return p.Next()
		}
		if err := p.updateMetricBytes(); err != nil {
			return promtextparse.EntryInvalid, err
	...
	default:
		return promtextparse.EntryInvalid, fmt.Errorf("invalid protobuf parsing state: %d", p.state)
	}
	return p.state, nil
}
...
func (p *ProtobufParser) getMagicName() string {
	t := p.mf.GetType()
	if p.state == promtextparse.EntryHistogram || (t != dto.MetricType_HISTOGRAM && t != dto.MetricType_GAUGE_HISTOGRAM && t != dto.MetricType_SUMMARY) {
		return p.mf.GetName()
	}
	if p.fieldPos == -2 {
...
func (p *ProtobufParser) getMagicLabel() (bool, string, string) {
	if p.state == promtextparse.EntryHistogram || p.fieldPos < 0 {
		return false, "", ""
	}
	switch p.mf.GetType() {
	case dto.MetricType_SUMMARY:
...
func readDelimited(b []byte, mf *dto.MetricFamily) (n int, err error) {
	...
	totalLength := varIntLength + int(messageLength) //nolint
