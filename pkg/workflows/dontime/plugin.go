...
func (p *Plugin) Observation(ctx context.Context, outctx ocr3types.OutcomeContext, query types.Query) (types.Observation, error) {
	...
	// inspect ObservedDonTimes to determine if the feature flag has taken effect
	var observedDonTimes *pb.ObservedDonTimes
	for _, observedDonTimes = range previousOutcome.ObservedDonTimes {
		break
	}
...
func (p *Plugin) unsequencedObservation(ctx context.Context, previousOutcome *pb.Outcome, query types.Query) (types.Observation, error) {
...
// sequencedObservation executes the updated Observation logic against the previous outcome's [pb.ObservedDonTimes.TimestampsBySequence].
