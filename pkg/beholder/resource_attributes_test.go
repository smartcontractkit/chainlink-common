...
func TestResourceAttributesToStringMap_Empty(t *testing.T) {
	// The no-attributes case is the common one; it returns nil rather than allocating.
	assert.Nil(t, resourceAttributesToStringMap(nil))
	assert.Nil(t, resourceAttributesToStringMap([]attribute.KeyValue{}))
}
