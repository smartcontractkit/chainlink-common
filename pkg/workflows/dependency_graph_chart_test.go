...
//go:generate go test -run=TestDependencyGraphFormatChart . -update
...
func requireEqualChart(t *testing.T, name string, workflow sdk.WorkflowSpec) {
	...
	graph, err := workflows.BuildDependencyGraph(workflow)
	require.NoError(t, err)
	s, err := graph.FormatChart()
	require.NoError(t, err)
...
// TestDependencyGraphFormatChart depends on charts golden files, and will regenerate them
// when the -update flag is used.
func TestDependencyGraphFormatChart(t *testing.T) {
	for _, tt := range []struct {
		...
		{"builder_parallel", buildSimpleWorkflowSpec(t,
...
func buildSimpleWorkflowSpec(t *testing.T, w *sdk.WorkflowSpecFactory) sdk.WorkflowSpec {
	...
	Consensus: []sdk.StepDefinition{
		{
			...
			Inputs: sdk.StepInputs{
				Mapping: map[string]any{"observations": []any{
					map[string]any{
