...
func IdenticalConsensus[T any](runner *testutils.Runner) *IdenticalConsensusMock[T] {
	consensus := &IdenticalConsensusMock[T]{
		Mock: testutils.MockCapability[[]T, ocr3.SignedReport]("offchain_reporting@1.0.0", identicalConsensus[T]),
	}
...
func identicalConsensus[T any](inputs []T) (ocr3.SignedReport, error) {
	if len(inputs) == 0 {
	...
	wrapped, err := values.Wrap(inputs[0])
	if err != nil {
...
type IdenticalConsensusMock[T any] struct {
	*testutils.Mock[[]T, ocr3.SignedReport]
}
...
func (c *IdenticalConsensusMock[T]) GetStepDecoded(ref string) testutils.StepResults[[]T, T] {
	step := c.GetStep(ref)
	var t T
	...
	return testutils.StepResults[[]T, T]{WasRun: step.WasRun, Input: step.Input, Output: t, Error: step.Error}
}
