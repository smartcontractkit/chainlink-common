...
// of them combine, and how a failure is reported are all properties of the map Files hands back.
func TestFiles(t *testing.T) {
	...
	t.Run("Returns what a generator produced, unformatted", func(t *testing.T) {
		...
		// Formatting and the header belong to the write, so a caller comparing against committed
		// bytes has to go through Run - see TestRun.
		require.Equal(t, map[string]string{"out/thing.go": "package out\nfunc  Thing ()  {}\n"}, files)
	})
	...
	t.Run("A generator's failure yields no files", func(t *testing.T) {
		dir := writePackage(t, module)
		files, err := Files(RunArgs{Dir: dir, Tool: "gen"}, Generator(func([]Package) (map[string]string, error) {
	...
	t.Run("Discovery failure is reported", func(t *testing.T) {
		_, err := Files(RunArgs{Dir: filepath.Join(t.TempDir(), "nowhere")})
		require.ErrorContains(t, err, "go.mod")
	})

	t.Run("An empty directory means the working directory", func(t *testing.T) {
		// This package's own directory encloses a module, so discovery resolves and finds nothing
		// to document without any roots.
		files, err := Files(RunArgs{Tool: "gen"}, Generator(func(pkgs []Package) (map[string]string, error) {
			require.Empty(t, pkgs)
			return nil, nil
		}))
		require.NoError(t, err)
...
// TestRun covers only what Files cannot: that the result reaches disk, formatted and headed the
// same way whichever generator produced it, and that a failure writes nothing at all.
