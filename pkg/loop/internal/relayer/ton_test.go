...
func Test_TONDomainRoundTripThroughGRPC(t *testing.T) {
	...
	t.Run("SendTransaction", func(t *testing.T) {
		...
		tonService.sendTransaction = func(ctx context.Context, m tontypes.Message) error {
			require.Equal(t, msg, m)
			return nil
		}
		err := client.SendTransaction(ctx, msg)
		require.NoError(t, err)
	})
	...
	t.Run("GetTransactionStatus", func(t *testing.T) {
		...
		status, ec, err := client.GetTransactionStatus(ctx, 999)
		require.NoError(t, err)
		require.Equal(t, txStatus, status)
		require.Equal(t, tontypes.ExitCode(0), ec)
	})

	t.Run("GetTransactionExecutionFees", func(t *testing.T) {
		...
		fee, err := client.GetTransactionExecutionFees(ctx, 123)
		require.NoError(t, err)
...
type staticTONService struct {
	...
	sendTransaction    func(ctx context.Context, msg tontypes.Message) error
...
func (s *staticTONService) SendTransaction(ctx context.Context, msg tontypes.Message) error {
	return s.sendTransaction(ctx, msg)
}
func (s *staticTONService) GetTransactionStatus(ctx context.Context, lt uint64) (types.TransactionStatus, tontypes.ExitCode, error) {
	return s.getTxStatus(ctx, lt)
}
func (s *staticTONService) GetTransactionExecutionFees(ctx context.Context, lt uint64) (*tontypes.TransactionFee, error) {
