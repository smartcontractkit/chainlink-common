...
import (
	...
	ocr3 "github.com/smartcontractkit/chainlink-common/pkg/loop/internal/pb/ocr3_1"
	ocr3_1 "github.com/smartcontractkit/chainlink-common/pkg/loop/internal/pb/ocr3_1"
)
...
func NewReportingPluginFactoryClient(b *net.BrokerExt, cc grpc.ClientConnInterface) *reportingPluginFactoryClient {
	b = b.WithName("OCR3_1ReportingPluginProviderClient")
...
var _ ocr3_1.ReportingPluginFactoryServer = (*reportingPluginFactoryServer)(nil)

type reportingPluginFactoryServer struct {
	ocr3_1.UnimplementedReportingPluginFactoryServer

	*net.BrokerExt

	impl ocr3_1types.ReportingPluginFactory[[]byte]
}

func NewReportingPluginFactoryServer(impl ocr3_1types.ReportingPluginFactory[[]byte], b *net.BrokerExt) *reportingPluginFactoryServer {
	return &reportingPluginFactoryServer{impl: impl, BrokerExt: b.WithName("OCR3ReportingPluginFactoryServer")}
}

func (r *reportingPluginFactoryServer) NewReportingPlugin(ctx context.Context, request *ocr3_1.NewReportingPluginRequest) (*ocr3_1.NewReportingPluginReply, error) {
	...
	rp, rpi, err := r.impl.NewReportingPlugin(ctx, cfg, nil)
	if err != nil {
		return nil, err
	}
	...
	return &ocr3_1.NewReportingPluginReply{ReportingPluginID: id, ReportingPluginInfo: &ocr3_1.ReportingPluginInfo{
		Name: rpi.Name,
		ReportingPluginLimits: &ocr3_1.ReportingPluginLimits{
...
func (o *reportingPluginClient) Query(ctx context.Context, seqNr uint64, keyValueReader ocr3_1types.KeyValueReader, blobBroadcastFetcher ocr3_1types.BlobBroadcastFetcher) (libocr.Query, error) {
	...
	reply, err := o.grpc.Query(ctx, &ocr3_1.QueryRequest{
...
func (o *reportingPluginClient) Observation(ctx context.Context, seqNr uint64, aq libocr.AttributedQuery, keyValueReader ocr3_1types.KeyValueReader, blobBroadcastFetcher ocr3_1types.BlobBroadcastFetcher) (libocr.Observation, error) {
	kvrID := o.keyValueReaders.Put(keyValueReader)
	defer o.keyValueReaders.Delete(kvrID)
	reply, err := o.grpc.Observation(ctx, &ocr3_1.ObservationRequest{
		KeyValueStateReaderID: kvrID,
		SeqNr:                 seqNr,
...
func (o *reportingPluginClient) Reports(ctx context.Context, seqNr uint64, outcome ocr3types.Outcome) ([]ocr3types.ReportPlus[[]byte], error) {
	...
	return reportsPlus(reply.ReportPlus), nil
}
...
func (o *reportingPluginServer) ObservationQuorum(ctx context.Context, request *ocr3.ObservationQuorumRequest) (*ocr3.ObservationQuorumReply, error) {
	...
	oq, err := o.impl.ObservationQuorum(ctx, outcomeContext(request.OutcomeContext), request.Query, aos)
	if err != nil {
		return nil, err
	}
...
func (o *reportingPluginServer) Reports(ctx context.Context, request *ocr3.ReportsRequest) (*ocr3.ReportsReply, error) {
	rp, err := o.impl.Reports(ctx, request.SeqNr, request.Outcome)
	if err != nil {
		return nil, err
	}
	return &ocr3.ReportsReply{
		ReportPlus: pbReportsPlus(rp),
	}, nil
}
...
func pbTransmissionSchedule(s *ocr3types.TransmissionSchedule) *ocr3.TransmissionSchedule {
	if s == nil {
		return nil
	}
	var pb ocr3.TransmissionSchedule
	for _, t := range s.Transmitters {
		pb.Transmitters = append(pb.Transmitters, uint32(t))
	}
	for _, td := range s.TransmissionDelays {
		pb.TransmissionDelays = append(pb.TransmissionDelays, int64(td))
	}
	return &pb
}
...
func reportsPlus(ri []*ocr3.ReportPlus) (rwi []ocr3types.ReportPlus[[]byte]) {
	for _, r := range ri {
		rwi = append(rwi, ocr3types.ReportPlus[[]byte]{
			ReportWithInfo: ocr3types.ReportWithInfo[[]byte]{
				Report: r.ReportWithInfo.Report,
				Info:   r.ReportWithInfo.Info,
			},
			TransmissionScheduleOverride: transmissionSchedule(r.TransmissionScheduleOverride),
		})
	}
	return
}

func transmissionSchedule(pb *ocr3.TransmissionSchedule) *ocr3types.TransmissionSchedule {
	if pb == nil {
		return nil
	}
	var ts ocr3types.TransmissionSchedule
	for _, t := range pb.Transmitters {
		ts.Transmitters = append(ts.Transmitters, commontypes.OracleID(t))
	}
	for _, td := range pb.TransmissionDelays {
		ts.TransmissionDelays = append(ts.TransmissionDelays, time.Duration(td))
	}
	return &ts
}
