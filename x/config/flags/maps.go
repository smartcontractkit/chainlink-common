...
// Go CLI library that binds maps of lists (its `mapsep` defaults to ';', its `sep` to ','); it is
// not pflag's own StringToString, which separates entries with ',' and so has no separator left
// for the elements of a list value. A ':' would collide with the values maps most often hold - a
// URL, a host:port, a timestamp - and no CLI uses it for this.
const (
	mapEntrySep = ";"
	mapKVSep    = "="
	// mapListSep splits a list value into its elements. It must stay in step with the separator
	// the list flag itself uses, which is what splits the value during decoding.
	mapListSep = ","
)

// textMapValue is the pflag.Value backing a map flag. It holds the entries as text and leaves
// converting them to the map's real key and value types to the decoder, so a flag, an env var and
// a config file all reach that decoder the same way.
//
// pflag's own StringToString is deliberately not used: viper special-cases that flag type and
// parses its value itself with a comma separator, which would both disagree with the syntax here
// and mangle a list value.
...
// to the decoder verbatim for any flag type it doesn't know. Entries are sorted so --help doesn't
...
// re-parsed: viper hands a flag's own string to the decoder rather than the entries behind it. A
// comma only needs quoting where it isn't already the list separator.
...
// Type is what --help prints after the flag name, so it spells out the syntax rather than naming
// a Go type nobody has to know. It must not be one of the flag types viper parses itself
// ("stringToString", "stringSlice", ...) - see the type doc.
...
// parseTextMap parses the "k=v;k=v" text form of a map into its entries. Values are left as text;
// the decoder converts each one to the map's actual value type, including splitting a list value
// on mapListSep. listValued says whether the target's values are lists, which is what makes a
// comma meaningful inside a value rather than a mistake.
func parseTextMap(s string, listValued bool) (map[string]string, error) {
	s = strings.TrimSpace(s)
	// The bracketed form is what String() prints, so a value read back from a flag's own default
	// (which viper does when nothing else set the key) round-trips.
	...
	for _, entry := range splitEntries(s) {
		...
		switch {
		case isQuoted(val):
			// An explicitly quoted value is taken whole, which is the way out for a scalar value
			// that really does contain a comma.
			val = strings.ReplaceAll(val[1:len(val)-1], `\"`, `"`)
		case listValued:
			// A list may also be bracketed, as AWS CLI shorthand writes one, so that a value reads
			// as a list even where the flag's own syntax already implies it.
...
// splitEntries splits the entries on mapEntrySep, ignoring a separator inside a quoted value so
// that a value holding one survives the trip out through String and back.
...
// map, so the syntax can't just accept both; storing "prod,region=us" as one value silently would
...
// textMapOf renders a map value as the entries a map flag holds, so a struct's compiled-in default
// survives into the flag's default and into --help. Returns nil for a nil or empty map, which
// prints as "[]".
...
func textOf(v reflect.Value) string {
	...
	// Map keys and values are not addressable, so a MarshalText declared on the pointer receiver
	// is invisible on the value itself; copy into an addressable pointer to find it.
