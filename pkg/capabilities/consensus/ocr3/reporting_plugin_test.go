...
func TestReportingPlugin_Reports_ShouldAcceptAttestedReport(t *testing.T) {
	ctx := tests.Context(t)
	lggr := logger.Test(t)
	s := newStore()
	cap := &mockCapability{
	...
	should, err := rp.ShouldAcceptAttestedReport(ctx, sqNr, rep)
	assert.False(t, should)
	require.NoError(t, err)

	resp := cap.gotResponse
	assert.Nil(t, resp.Err)

	gotM := resp.Value.(*values.Map)
	unwrapped, err := values.Unwrap(gotM.Underlying["report"])
	require.NoError(t, err)
	assert.Equal(t, unwrapped, spb)
}

func TestReportingPlugin_Reports_ShouldAcceptAttestedReport_ShouldReportFalse(t *testing.T) {
	ctx := tests.Context(t)
	lggr := logger.Test(t)
	s := newStore()
	cap := &mockCapability{
	...
	gotM := resp.Value.(*values.Map)
	assert.Equal(t, gotM.Underlying["report"], values.Value(nil))
}
