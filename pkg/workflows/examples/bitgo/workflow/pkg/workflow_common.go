...
func onTrigger(runtime sdk.DonRuntime, executionTime int64, config *Config) (struct{}, error) {
	logger := slog.Default()
	...
	if reserveInfo.LastUpdated.Before(time.Unix(executionTime, 0).Add(-time.Hour * 24)) {
