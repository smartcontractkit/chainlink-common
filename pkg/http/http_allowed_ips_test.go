...
func TestHttpAllowedIPS_isRestrictedIP(t *testing.T) {
	...
	t.Run("disallows queries to database IP", func(t *testing.T) {
		...
		t.Cleanup(s.Close)
		u := config.MustParseURL(s.URL)
		r, err := isRestrictedIP(net.ParseIP(u.Host), testDBURLcfg{*u.URL()})
		require.NoError(t, err)
		assert.True(t, r)
	})

	t.Run("errors on failed lookup", func(t *testing.T) {
		u := config.MustParseURL("postgresql://postgres@1.2.3.4:5432/chainlink_test?sslmode=disable")
		_, err := isRestrictedIP(net.ParseIP("1.2.3.4"), testDBURLcfg{*u.URL()})
		require.Error(t, err)
	})
}
