...
type CronCapability interface {
	RegisterTrigger(ctx context.Context, triggerID string, metadata capabilities.RequestMetadata, input *cron.Config) (<-chan capabilities.TriggerAndId[*cron.CronTrigger], error)
...
func (c *cronCapability) Info(ctx context.Context) (capabilities.CapabilityInfo, error) {
	...
	return capabilities.NewCapabilityInfo("cron@1.0.0", capabilities.CapabilityTypeCombined, c.CronCapability.Description())
}
...
func (c *cronCapability) RegisterTrigger(ctx context.Context, request capabilities.TriggerRegistrationRequest) (<-chan capabilities.TriggerResponse, error) {
	...
	case "Trigger":
		input := &cron.Config{}
		return capabilities.RegisterTrigger(ctx, c.stopCh, "cron@1.0.0", request, input, c.CronCapability.RegisterTrigger)
	default:
