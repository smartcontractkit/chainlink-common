...
func (r *RelayerService) NewContractStateReader(ctx context.Context, config []byte) (types.ContractStateReader, error) {
	if err := r.WaitCtx(ctx); err != nil {
		return nil, err
	}
	return r.Service.NewContractStateReader(ctx, config)
}
