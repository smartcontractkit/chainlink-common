...
func (s *Setting[T]) initSetting(key string, scope Scope) error {
	s.Key = key
	s.Scope = scope
	if s.Parse == nil {
...
func Duration(defaultValue time.Duration) Setting[time.Duration] {
	return NewSetting(defaultValue, time.ParseDuration)
}
...
func Rate(defaultLimit rate.Limit, defaultBurst int) Setting[config.Rate] {
	return NewSetting(config.Rate{Limit: defaultLimit, Burst: defaultBurst}, config.ParseRate)
}

func Size(defaultValue config.Size) Setting[config.Size] {
	return NewSetting(defaultValue, config.ParseByte)
}
...
// InitKeys accepts a pointer to a struct and iterates over all the fields, initializing any Setting fields with a full-qualified, dot-separated key.
// Every field must either be a Setting or a nested struct following the same rules.
func InitKeys(a any) error {
	return initKeys(ScopeGlobal, "", a)
}

func initKeys(scope Scope, parent string, a any) error {
	v := reflect.ValueOf(a)
	...
	for i := range v.NumField() {
		...
		if s, ok := f.Addr().Interface().(keySetter); ok {
			if err := s.initSetting(key, scope); err != nil {
				return fmt.Errorf("%s failed to init Setting: %s", key, err)
			}
		} else if f.Type().Kind() == reflect.Struct {
			if err := initKeys(scope, key, f.Addr().Interface()); err != nil {
...
type keySetter interface {
	initSetting(key string, scope Scope) error
}
