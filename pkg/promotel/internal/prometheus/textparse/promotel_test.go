...
func createTestProtoBufGatherer(t *testing.T) *testGatherer {
	testMetricFamilies, _ := createTestProtoBuf(t)

	metrics := make([]*io_prometheus_client.MetricFamily, 0, len(testMetricFamilies))
...
func TestProtobufParseShim(t *testing.T) {
	...
	testPromGatherer := createTestProtoBufGatherer(t)

	metrics, err := testPromGatherer.Gather()
	require.NoError(t, err)
	scenarios := []struct {
		name     string
		parser   promtextparse.Parser
		expected []parseResult
	}{
		{
			name:   "ignore classic buckets of native histograms",
			parser: NewProtobufParserShim(false, labels.NewSymbolTable(), metrics),
			expected: []parseResult{
				{
					m:    "go_build_info",
		...
		},
		{
			...
			parser: NewProtobufParserShim(true, labels.NewSymbolTable(), metrics),
			expected: []parseResult{
				{ // 0
					m:    "go_build_info",
					help: "Build information about the main Go module.",
	...
	for _, scenario := range scenarios {
		t.Run(scenario.name, func(t *testing.T) {
			...
			for {
				...
				switch et {
				case promtextparse.EntrySeries:
					m, ts, v := p.Series()
					...
					require.Equal(t, exp[i].v, v, "i: %d", i) // nolint
				...
				case promtextparse.EntryHistogram:
				...
				case promtextparse.EntryUnit:
					m, u := p.Unit()
					require.Equal(t, exp[i].m, string(m), "i: %d", i)
					require.Equal(t, exp[i].unit, string(u), "i: %d", i)

				case promtextparse.EntryComment:
