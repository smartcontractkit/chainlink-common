...
import (
	...
	"github.com/smartcontractkit/chainlink-common/pkg/loop/internal/core/services/keyvaluestore"
...
func (m *ReportingPluginServiceClient) NewReportingPluginFactory(
	...
	cc := m.NewClientConn("ReportingPluginServiceFactory", func(ctx context.Context) (id uint32, deps net.Resources, err error) {
		...
		keyValueStoreID, keyValueStoreRes, err := m.ServeNew("KeyValueStore", func(s *grpc.Server) {
			pb.RegisterKeyValueStoreServer(s, keyvaluestore.NewServer(keyValueStore))
		})
...
func (m *reportingPluginServiceServer) NewReportingPluginFactory(ctx context.Context, request *pb.NewReportingPluginFactoryRequest) (*pb.NewReportingPluginFactoryReply, error) {
	...
	keyValueStore := keyvaluestore.NewClient(keyValueStoreConn)
	...
	factory, err := m.impl.NewReportingPluginFactory(ctx, config, providerConn, pipelineRunner, telemetry, errorLog, keyValueStore)
	if err != nil {
		m.CloseAll(providerRes, errorLogRes, pipelineRunnerRes, telemetryRes, keyValueStoreRes)
		return nil, err
	}
