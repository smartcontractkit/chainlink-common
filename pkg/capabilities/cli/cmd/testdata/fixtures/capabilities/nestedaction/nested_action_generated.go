...
type ActionOutputs struct {
	...
	Results ActionOutputsResults `json:"results" yaml:"results" mapstructure:"results"`
}
...
// UnmarshalJSON implements json.Unmarshaler.
func (j *ActionOutputsResults) UnmarshalJSON(b []byte) error {
	var raw map[string]interface{}
	if err := json.Unmarshal(b, &raw); err != nil {
		return err
	}
	if _, ok := raw["adapted_thing"]; raw != nil && !ok {
		return fmt.Errorf("field adapted_thing in ActionOutputsResults: required")
	}
	type Plain ActionOutputsResults
	var plain Plain
	if err := json.Unmarshal(b, &plain); err != nil {
		return err
	}
	*j = ActionOutputsResults(plain)
	return nil
}
...
func (j *ActionOutputs) UnmarshalJSON(b []byte) error {
	...
	if _, ok := raw["results"]; raw != nil && !ok {
		return fmt.Errorf("field results in ActionOutputs: required")
	}
