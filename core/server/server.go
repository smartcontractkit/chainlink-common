...
import (
	...
	"github.com/smartcontractkit/chainlink-relay/core/server/adapter"
	"github.com/smartcontractkit/chainlink-relay/core/server/keys"
...
func RunWebserver(
	eiCfg config.WebhookConfig,
	store types.SubscriptionStorer,
	port int,
	services types.ServicesPipeline,
	pubKeys *map[string]string,
) {
	srv := NewHTTPService(eiCfg, store, services, pubKeys)
...
func (srv *HttpService) createRouter() {
	...
	// endpoint implementation for fetching and setting key data
	keys := keys.New(srv.pubKeys)
	engine.GET("/keys", keys.ShowKeys)
	engine.POST("/keys", keys.SetEIKeys)

	// webhook implementation
	wh := webhook.New(srv.Store, srv.services)
	authJob := engine.Group("/jobs")
	authJob.Use(webhook.Authenticate(srv.cfg))
	{
