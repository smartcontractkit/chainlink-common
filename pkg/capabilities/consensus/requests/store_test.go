...
func TestOCR3Store(t *testing.T) {
	...
	s := requests.NewStore[*testRequest]()
	rid := uuid.New().String()
	req := &testRequest{
	...
	t.Run("firstN, batchSize larger than queue", func(t *testing.T) {
		for range 10 {
			err := s.Add(&testRequest{WorkflowExecutionID: uuid.New().String(), ExpiresAt: n.Add(1 * time.Hour)})
			require.NoError(t, err)
		}
...
func TestOCR3Store_ReadRequestsCopy(t *testing.T) {
	...
	testCases := []struct {
		name string
		get  func(ctx context.Context, rid string) *testRequest
	}{
		{
			name: "get",
			get: func(ctx context.Context, rid string) *testRequest {
