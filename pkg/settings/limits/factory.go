...
// If Meter is set, the following metrics will be emitted
//  - time.*.limit - int gauge
//  - time.*.usage - int gauge
...
// NewResourceLimiter returns a ResourceLimiter for the given limit, and configured by the Factory.
// If Meter is set, the following metrics will be emitted
//  - resource.*.limit - gauge
//  - resource.*.usage - gauge
func NewResourceLimiter[N Number](f Factory, limit settings.Setting[N]) (ResourceLimiter[N], error) {
	if limit.Scope == settings.ScopeGlobal {
		return newGlobalResourceLimiter(f, limit)
	}
	return newScopedResourceLimiter(f, limit)
}
