...
// AdditionalFile represents a file to be copied during installation
...
type PluginDef struct {
	...
	AdditionalFiles []AdditionalFile `yaml:"additionalFiles"`
}
...
func downloadAndInstallPlugin(pluginType string, pluginIdx int, plugin PluginDef, defaults DefaultsConfig, manifestDir string) error {
...
func writeBuildManifest(tasks []PluginInstallTask, outputDir string) error {
	...
	for _, task := range tasks {
		// Convert additionalFiles to use raw paths without escaping
		additionalFiles := make([]AdditionalFile, len(task.Plugin.AdditionalFiles))
		for i, file := range task.Plugin.AdditionalFiles {
			additionalFiles[i] = AdditionalFile{
				Src:  file.Src,
				Dest: file.Dest,
			}
		}

		manifest.Plugins[task.PluginType] = PluginManifest{
	...
	manifestFile := filepath.Join(outputDir, "build-manifest.json")
	...
	log.Printf("Wrote build manifest to %s", manifestFile)
	return nil
}
...
func printHelp() {
	...
	fmt.Println("  -m, --manifest-dir <path>  Directory for additional files manifests")
...
func installPlugins(tasks []PluginInstallTask, concurrency int, verbose bool, outputDir string) error {
	if len(tasks) == 0 {
	...
	// Write build manifest first
	if err := writeBuildManifest(tasks, outputDir); err != nil {
		return fmt.Errorf("failed to write build manifest: %w", err)
	}
		...
		go func(workerID int) {
			...
			for task := range taskCh {
				...
				err := downloadAndInstallPlugin(task.PluginType, 0, task.Plugin, task.Defaults,
					filepath.Join(outputDir, "additional_files"))
	...
	if hasErrors {
		...
		// Replace fmt.Errorf with errors.New for simple string errors
...
// setupPaths ensures output directories exist
func setupPaths(manifestDir, outputDir string) (string, string, error) {
	...
	if !filepath.IsAbs(manifestDir) {
		...
		manifestDir = filepath.Join(wd, manifestDir)
	}
	...
	additionalFilesDir := filepath.Join(outputDir, "additional_files")
	if err := os.MkdirAll(additionalFilesDir, 0755); err != nil {
		return "", "", fmt.Errorf("failed to create additional files directory: %w", err)
	}

	return manifestDir, outputDir, nil
}
...
func main() {
	...
	flag.StringVar(&manifestDir, "manifest-dir", "docker/additional_files", "Directory for additional files manifests")
	flag.StringVar(&manifestDir, "m", "docker/additional_files", "Directory for additional files manifests (shorthand)")
	flag.StringVar(&outputDir, "output-dir", "docker/output_manifests", "Directory for output manifests")
	flag.StringVar(&outputDir, "o", "docker/output_manifests", "Directory for output manifests (shorthand)")
	...
	var err error
	manifestDir, outputDir, err = setupPaths(manifestDir, outputDir)
	if err != nil {
		log.Fatalf("Failed to setup paths: %v", err)
	}
	...
	// Install all plugins
	if err := installPlugins(allTasks, concurrency, verbose, outputDir); err != nil {
		os.Exit(1)
	}
}
