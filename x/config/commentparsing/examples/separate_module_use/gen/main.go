...
//
// Run writes only into the directory this module owns. The dependency's types are resolved from
// the DocComments compiled into it, since nothing here can reach that source.
package main
...
// reference is a second generator, fed the same packages Run discovered. The dependency's
// comments arrive here having crossed a module boundary compiled.
func reference(pkgs []commentparsing.Package) (map[string]string, error) {
	...
	for _, pkg := range pkgs {
		...
		for _, typ := range pkg.Types {
			...
			// Sorted rather than taken from the map, so regenerating an unchanged package
