...
func (a *atomicTriggerCapability) AckEvent(ctx context.Context, triggerId string, eventId string, workflowId string) error {
	a.mu.Lock()
	defer a.mu.Unlock()
	if a.cap == nil {
		return errors.New("capability unavailable")
	}
	return a.cap.AckEvent(ctx, triggerId, eventId, workflowId)
}
...
func (a *atomicExecuteAndTriggerCapability) AckEvent(ctx context.Context, triggerId string, eventId string, workflowId string) error {
	a.mu.Lock()
	defer a.mu.Unlock()
	if a.cap == nil {
		return errors.New("capability unavailable")
