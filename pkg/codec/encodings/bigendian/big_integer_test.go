...
func TestBigInteger(t *testing.T) {
	t.Run("NewBigInt returns error if numBytes is too large", func(t *testing.T) {
		_, err := bigendian.NewBigInt(bigbigendian.MaxSize+1, false)
		assert.True(t, errors.Is(err, types.ErrInvalidConfig))
	})

	t.Run("NewBigInt returns error if numBytes is -ve", func(t *testing.T) {
		_, err := bigendian.NewBigInt(-1, false)
		assert.True(t, errors.Is(err, types.ErrInvalidConfig))
	})

	signed, createErr := bigendian.NewBigInt(4, true)
	require.NoError(t, createErr)
	unsigned, createErr := bigendian.NewBigInt(4, false)
	require.NoError(t, createErr)
	t.Run("Encode and Decode work together on signed values", func(t *testing.T) {
		...
		expected, err := bigendian.Int32{}.Encode(int32(anyVal.Int64()), nil)
		require.NoError(t, err)
	...
	t.Run("Encode and Decode work together on unsigned values", func(t *testing.T) {
		...
		require.NoError(t, err)
		expected, err := bigendian.Uint32{}.Encode(uint32(anyVal.Uint64()), nil)
		require.NoError(t, err)
		assert.Equal(t, expected, bytes)
