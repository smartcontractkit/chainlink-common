...
type NodeJWTAuthenticatorConfig struct {
	// Logger is the chainlink logger to use. If nil, logger.Nop() is used.
...
func (cfg NodeJWTAuthenticatorConfig) New(nodeAuthProvider NodeAuthProvider) *NodeJWTAuthenticator {
	lggr := cfg.Logger
	if lggr == nil {
		lggr = logger.Nop()
	}
...
func NewNodeJWTAuthenticator(nodeAuthProvider NodeAuthProvider, slogger *slog.Logger, config ...*NodeJWTAuthenticatorConfig) *NodeJWTAuthenticator {
	cfg := NodeJWTAuthenticatorConfig{Logger: newSlogLoggerAdapter(slogger)}
	for _, c := range config {
		if c != nil {
			cfg.Leeway = c.Leeway
		}
	}
	return cfg.New(nodeAuthProvider)
}
...
func NewNodeJWTAuthenticatorWithLogger(nodeAuthProvider NodeAuthProvider, lggr logger.Logger, config ...*NodeJWTAuthenticatorConfig) *NodeJWTAuthenticator {
	cfg := NodeJWTAuthenticatorConfig{Logger: lggr}
	for _, c := range config {
		if c != nil {
			cfg.Leeway = c.Leeway
		}
