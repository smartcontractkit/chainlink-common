...
func NewString(maxLength uint) (encodings.TypeCodec, error) {
	headerSize := (bits.Len(maxLength) + 7) / 8
	sizeEncoder, err := GetIntTypeCodecByByteSize(headerSize)
	if err != nil {
		return nil, err
	}

	codec, err := encodings.NewSlice(Uint8{}, sizeEncoder)
	if err != nil {
		return nil, err
	}
