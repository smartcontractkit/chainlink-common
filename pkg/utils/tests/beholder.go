...
// Len asserts the total number of messages received that match the provided attribute key/value pairs.
func (b BeholderTester) Len(t *testing.T, length int, attrKVs ...any) bool {
	t.Helper()
	...
	for _, eMsg := range b.emitter.msgs {
		...
		for i < len(attrKVs)-1 {
			...
			if val, ok := eMsg.Attrs[key]; ok && reflect.DeepEqual(value, val) {
	...
	return assert.Len(t, found, length)
}
