...
type ClientClient interface {
	...
	LogTrigger(ctx context.Context, in *FilterLogTriggerRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[evm.Log], error)
}
...
func (c *clientClient) LogTrigger(ctx context.Context, in *FilterLogTriggerRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[evm.Log], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &Client_ServiceDesc.Streams[0], Client_LogTrigger_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[FilterLogTriggerRequest, evm.Log]{ClientStream: stream}
...
type Client_LogTriggerClient = grpc.ServerStreamingClient[evm.Log]
...
type ClientServer interface {
	...
	LogTrigger(*FilterLogTriggerRequest, grpc.ServerStreamingServer[evm.Log]) error
...
func (UnimplementedClientServer) LogTrigger(*FilterLogTriggerRequest, grpc.ServerStreamingServer[evm.Log]) error {
...
func _Client_LogTrigger_Handler(srv interface{}, stream grpc.ServerStream) error {
	...
	return srv.(ClientServer).LogTrigger(m, &grpc.GenericServerStream[FilterLogTriggerRequest, evm.Log]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type Client_LogTriggerServer = grpc.ServerStreamingServer[evm.Log]
