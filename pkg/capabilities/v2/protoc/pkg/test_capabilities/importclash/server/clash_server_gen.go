...
func (c *BasicActionServer) Initialise(ctx context.Context, dependencies core.StandardCapabilitiesDependencies) error {
	...
	initAttrs := []attribute.KeyValue{attribute.String("capability", "import-clash@1.0.0")}
	...
	if err := dependencies.CapabilityRegistry.Add(ctx, &basicActionCapability{
		...
		return fmt.Errorf("error when adding %s to the registry: %w", "import-clash@1.0.0", err)
	}
...
func (c *BasicActionServer) Close() error {
	...
	if c.capabilityRegistry != nil {
		if err := c.capabilityRegistry.Remove(ctx, "import-clash@1.0.0"); err != nil {
			return err
		}
	}
...
func (c *basicActionCapability) Execute(ctx context.Context, request capabilities.CapabilityRequest) (capabilities.CapabilityResponse, error) {
	...
	case "PerformAction":
		...
		wrapped := func(ctx context.Context, metadata capabilities.RequestMetadata, input *p1.Item, _ *emptypb.Empty) (*p2.Item, capabilities.ResponseMetadata, *capabilities.OCRAttestation, error) {
