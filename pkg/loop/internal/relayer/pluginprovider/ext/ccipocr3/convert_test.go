...
// TestMapConversionUsingValuesPackage tests the conversion between Go maps and protobuf Maps using the values package
func TestMapConversionUsingValuesPackage(t *testing.T) {
	tests := []struct {
		name     string
		...
		},
		{
			...
			expected: map[string]any{
				"nested_map": map[string]any{
					"inner_key": "inner_value",
					"inner_num": int64(100), // Note: values package might convert different int types to int64
				},
				"list": []any{
					"item1",
					int64(200),
