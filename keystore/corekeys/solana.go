...
func (ks *Store) GenerateEncryptedSolanaKey(ctx context.Context, password string) ([]byte, error) {
	path := keystore.NewKeyPath(TypeSolana, nameDefault)
...
func FromEncryptedSolanaKey(data []byte, password string) ([]byte, error) {
	...
	if envelope.Type != TypeSolana {
		return nil, fmt.Errorf("invalid key type: expected %s, got %s", TypeSolana, envelope.Type)
	}
